  ...
  <body>
    ...
    <script>
        ...
        function setTaxiStand_Marker(taxiStandList){    // Take in Array
            ...
            for(var i = 0; i < taxiStandList.length; i++){
                var curLat = parseFloat(taxiStandList[i][4]);
                var curLng = parseFloat(taxiStandList[i][5]);
                var taxiStandMarker = new google.maps.Marker({                    
                    position: {lat: curLat, lng: curLng},     
                    animation: google.maps.Animation.DROP, // Have BOUNCE
                    icon: taxiStand_marker,
                    map: map
                });
                
                overlayMarkers.push(taxiStandMarker);    
                
                var infowindow = new google.maps.InfoWindow();
                
                var infoContent = "<p>Taxi Stand : <span style='font-weight:bold'>" + taxiStandList[i][1] + "</span></p><p>In Queue : <span style='font-weight:bold'>" + taxiStandList[i][2] + "</span></p></p><p>Taxi Waiting : <span style='font-weight:bold'>" + taxiStandList[i][3] + "</span></p>";
                google.maps.event.addListener(taxiStandMarker,'click', (function(marker,content,infowindow){ 
                    return function() {
                        closeInfos();
                        infowindow.setContent(content);
                        infowindow.open(map,marker);
                ...
                })(taxiStandMarker,infoContent,infowindow)); 
        ...
        function getTaxiStandWithinRadius(currentPoint){
            selectedTaxiQueue = [];
            for(var bs in taxiQueue){
                var tempLat = parseFloat(taxiQueue[bs][4]);
                var tempLng = parseFloat(taxiQueue[bs][5]);
                if(searchRadius > distanceBetweenPoints(currentPoint.lat(), currentPoint.lng(), tempLat, tempLng)){
                    selectedTaxiQueue.push(taxiQueue[bs]);                    
                }                
            }           
        ...
        function initMap() {       
            ...
            google.maps.event.addListener(map, 'click', function(event){
                ...
                if(taxiStandCheck == true){
                    getTaxiStandWithinRadius(event.latLng);
                    setTaxiStand_Marker(selectedTaxiQueue);
                }
        ...
        //Taxi Queue
        <%
            for(int x = 0 ; x < taxiQ.size(); x++){
        %>
                var line = "<%= taxiQ.get(x) %>";
                var lineArr = line.split(",");
                taxiQueue.push(lineArr);               
        <%
            }
        %>
